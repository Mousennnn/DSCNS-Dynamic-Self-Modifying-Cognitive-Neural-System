...
## 9. Phase 5.1 �� mandatory self-modification + error correction (v0.4.0)
...
- **Memory retrieval is cosine-only** �� no learned retrieval; the top-k
  selection may not find the most relevant experiences for a given state.
- **Single seed, single network, single model** �� same limitations as P5.
- **No claim** that the system "understands" its errors or "intentionally"
  corrects itself �� the evidence shows error-conditioned behavior change,
  not comprehension.
