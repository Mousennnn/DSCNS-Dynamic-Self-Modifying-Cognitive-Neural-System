...
## 12. Phase 5.1 附录：强制自修改 + 自主幅度 + 错误条件自纠正

在 P5 基础上扩展三项机制（v0.4.0）：

- **强制修改**：每轮必须产生非零 Δθ（m_t ≥ 0.02，由 Magnitude Head 自主生成）
- **目标选择**：Target Head 3 类 softmax 选择修改目标组
- **错误条件学习**：ErrorEncoder 编码修改后果 → P_φ 未来决策受此条件化

**150 轮消融实验**（5 臂、冻结探针集、seed 42）：

| 臂 | 净漂移 D_net | 探针漂移 | 幅度 m_t | 成功/失败 |
|---|---|---|---|---|
| no_mod（冻结） | 0.000 | 0.000 | 0.000 | 150/0 |
| p5_m（P5 基线） | 42.46 | 0.437 | 1.000 | 150/0 |
| p5_mm（强制+自主幅度） | 0.607 | 0.011 | 0.054 | 150/0 |
| p5_mme（+错误学习） | **2.350** | 0.040 | **0.203** | 150/0 |
| random（预算匹配） | 3.766 | 0.043 | 0.500 | 150/0 |

**核心发现**：错误编码器将有效漂移从 0.61 提升到 2.35（3.8×），幅度从 0.054 升至 0.203——错误条件确实影响了模型的修改行为。在保守 α 下无失败，错误纠正机制已就位但未完全触发。

**诚实边界**：错误条件化行为 ≠ 理解错误；单种子、单网络、原型尺度。详见 `docs/PHASE5_1.md`。

---

**复现完成日期：** 2026-08-18（Phase 0–5）；2026-08-18（Phase 5.1 强制自修改 + 错误条件自纠正）
**结论：** 系统框架（经验→观察→评估→验证→内化→记忆→演化闭环）完整可运行；H3 与 Phase-2 信息增益采样得到支持，H1/H2 的原型尺度验证结果不支持（并给出原因）；Phase 4 实现了"自状态→学习式策略→结构动作→候选→评估→奖励→策略更新"的闭环（单种子原型演示）；Phase 5 实现了"θ→h→Δθ→θ′"内生参数自修改闭环并通过全部核心验证与负对照（单种子原型演示）；Phase 5.1 实现了"强制修改+自主幅度+错误条件学习"扩展，150轮消融验证错误条件确实影响修改行为（3.8×漂移增长），不声称"理解错误"（单种子原型演示）；所有实验数据与日志可复现、可追溯。
